page 74049 "MCH Maint. Location Logo FactB"
{
    Caption = 'Logo';
    Editable = false;
    LinksAllowed = false;
    ...
    actions
    {
        area(processing)
        {
            ToolTip = 'Specifies a company logo for the maintenance location. The picture in Company Information will be used if no logo specified.';
    ...
    begin
        Find;
        TestField(Code);
        AMMediaMgt.MaintLocationImportPictureFromFile(Rec);
    end;
    ...
    begin
        ...
        if not Picture.HasValue then
          exit;
        TestField(Code);
        AMMediaMgt.MaintLocationExportPictureToFile(Rec);
    end;
