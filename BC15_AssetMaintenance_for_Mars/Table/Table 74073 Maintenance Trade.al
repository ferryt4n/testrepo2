table 74073 "MCH Maintenance Trade"
{
    Caption = 'Maintenance Trade';
    DataCaptionFields = "Code",Description;
    DrillDownPageID = "MCH Maintenance Trade List";
    LookupPageID = "MCH Maintenance Trade Lookup";
    ...
    fields
    {
        ...
        field(5;"Item No.";Code[20])
        {
            ...
            begin
                if ("Item No." <> xRec."Item No.") then begin
                  ...
                  if ("Item No." <> '') and (CurrFieldNo = FieldNo("Item No.")) then begin
                    GetItem(Item);
                    if (Item."Purch. Unit of Measure" <> '') then begin
                      if not AMFunctions.TestIfUOMTimeBased(Item."Purch. Unit of Measure",false) then
                        Message(
                          IsNotTimeBasedUOMMsg+IsNotTimeBasedUOMMsg2,
                          Item.FieldCaption("Purch. Unit of Measure"),Item."Purch. Unit of Measure",
                          Item.TableCaption,Item."No.",TableCaption,Code,FieldCaption("Maint. Unit of Measure"));
                    end;
                  end;
                end;
            end;
        ...
        {
            CalcFormula = Exist("MCH AM Comment Line" WHERE ("Table Name"=CONST(Trade),
                                                             "No."=FIELD(Code)));
            Caption = 'Comment';
            Editable = false;
            FieldClass = FlowField;
        ...
        field(150;"Maint. Unit of Measure";Code[10])
        {
            ...
            trigger OnValidate()
            begin
                AMFunctions.TestIfUOMTimeBased("Maint. Unit of Measure",true);
            end;
        }
    ...
    begin
        AMaintLedgEntry.SetRange(Type,AMaintLedgEntry.Type::Trade);
        ...
        MaintTaskBudgetLine.SetRange(Type,MaintTaskBudgetLine.Type::Trade);
        ...
        WorkOrderBudgetLine.SetRange(Type,WorkOrderBudgetLine.Type::Trade);
        ...
        MaintCommentLine.SetRange("Table Name",MaintCommentLine."Table Name"::Trade);
    ...
    var
        ...
        IsNotTimeBasedUOMMsg: Label 'The %1 (%2) of %3 %4 in %5 %6 must be a Time based unit of measure.\';
        IsNotTimeBasedUOMMsg2: Label 'Select a different Item or specify a valid %7.';
    ...
    begin
        ...
        if CheckItem then begin
          ...
          if (Item."Purch. Unit of Measure" <> '') then
            AMFunctions.TestIfUOMTimeBased(Item."Purch. Unit of Measure",true)
          else
            AMFunctions.TestIfUOMTimeBased(Item."Base Unit of Measure",true);
        end;
    end;
